...
const initKakaoMap = () => {
  const courseHtml = document.querySelectorAll(".course");
  // 데이터 가져오기
  for (let i = 0; i < courseHtml.length; i++) {
    let courseNo = 0;
    if (courseHtml[i].dataset.courseNo !== "0") {
      courseNo = courseHtml[i].dataset.courseNo;
      courseData.push({
        course_no: courseHtml[i].dataset.courseNo,
        course_latitude: courseHtml[i].dataset.courseLatitude,
        course_longitude: courseHtml[i].dataset.courseLongitude
      });
    }
    
    // 클릭이벤트
    courseHtml[i].addEventListener("click", (e) => clickCourseList(e, courseNo));
  }
